...
const TestArray = [{ name: "test1" }, { name: "test2" }, { name: "test3" }, { name: "test4" }];

const answer = "Link is added";

async function getLink2Map(key, chatId) {
  ...
  if (request.method === "POST") {
    ...
    if (isCommand) {
      ...
      if (command === "/all") {
        ...
        Promise.all(
          links.keys.map(async (i) => {
            const requestedLink = await LINKS.get(i.name);
            const parsedData = JSON.parse(requestedLink);

            if (!allChatIds.includes(parsedData.id)) {
              allChatIds.push(parsedData.id);
            }
          })
        );

        Promise.all(
          allChatIds.map(async (i) => {
            await getAllLinks(i);
          })
        );
        const url = `https://api.telegram.org/bot${API_KEY}/sendMessage?chat_id=246389668&text=all your links from this week`;
        await fetch(url); // No need to parse response
      } else if (command === "/del") {
